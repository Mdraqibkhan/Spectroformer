...
# 		self.conv_1 = nn.Conv2d(channels, 2*channels, kernel_size=3, padding=1,groups=channels, bias=True)
...
# 		super(FrequencyFeature, self).__init__()
# 		self.conv_amplitude1 = nn.Conv2d(in_channels, 2*in_channels, kernel_size=1,groups=in_channels)
# 		self.conv_phase1 = nn.Conv2d(in_channels, 2*in_channels, kernel_size=1,groups=in_channels)
# 		self.conv_amplitude2 = nn.Conv2d(2*in_channels, in_channels, kernel_size=1,groups=in_channels)
# 		self.conv_phase2 = nn.Conv2d(2*in_channels, in_channels, kernel_size=1,groups=in_channels)
		
# 	def forward(self, x):
# 		x_fft = fft.fftn(x, dim=(-2, -1)) # Apply FFT to the input tensor
# 		x_amplitude = torch.abs(x_fft) # Separate amplitude
# 		x_phase = torch.angle(x_fft) # Separate phase
# 		x_amplitude1 = self.conv_amplitude1(x_amplitude) # Apply convolution to amplitude
# 		x_phase1 = self.conv_phase1(x_phase) # Apply convolution to phase
# 		x_amplitude2 = self.conv_amplitude2(x_amplitude1) # Apply convolution to amplitude
# 		x_phase2 = self.conv_phase2(x_phase1) # Apply convolution to phase


# 		x_reconstructed = fft.ifftn(x_amplitude2 * torch.exp(1j * x_phase2), dim=(-2, -1)).real # Reconstruct the tensor by taking IFFT
...
# 	   ######FREQUENCY FATURE#################
# 		self.conv_amplitude1 = nn.Conv2d(channels, 2*channels, kernel_size=1)
# 		self.conv_phase1 = nn.Conv2d(channels, 2*channels, kernel_size=1)
# 		self.conv_amplitude2 = nn.Conv2d(2*channels, channels, kernel_size=1)
# 		self.conv_phase2 = nn.Conv2d(2*channels, channels, kernel_size=1)
# 		self.sig=nn.Sigmoid()
# 		self.bn=nn.BatchNorm2d(channels)
# 		self.con=nn.Conv2d(2*channels, channels, 1, 1, 0)
...
# 		x_reconstructed = fft.ifftn(x_amplitude2 * torch.exp(1j * x_phase2), dim=(-2, -1)).real # Reconstruct the tensor by taking IFFT
# 		x_reconstructed +=x
# 		x_reconstructed=self.bn(x_reconstructed)
# 		f_out=torch.cat([out,x_reconstructed],dim=1)
# 		f_out=self.con(f_out)
# 		return x*self.sig(f_out)
...
# 		self.SP,self.FP=SpatialFeature(channels),FrequencyFeature(channels)
# 		# self.add=AddFeatures()
# 		self.qkv = nn.Conv2d(channels, channels * 3, kernel_size=1, bias=False)
# 		self.qkv_conv = nn.Conv2d(channels * 3, channels * 3, kernel_size=3, padding=1, groups=channels * 3, bias=False)
# 		self.project_out = nn.Conv2d(channels, channels, kernel_size=1, bias=False)
# 		self.project_out1 = nn.Conv2d(2*channels, channels, kernel_size=1, bias=False)


# 	def forward(self, x):
# 		b, c, h, w = x.shape
# 		q, k, v = self.qkv_conv(self.qkv(x)).chunk(3, dim=1)
# 		q,k,v= self.project_out1(torch.cat([self.SP(q),self.FP(q)],dim=1)),self.project_out1(torch.cat([self.SP(q),self.FP(q)],dim=1)),self.project_out1(torch.cat([self.SP(q),self.FP(q)],dim=1))	
...
# 		self.ups_1=UpSample(128,True)
# 		self.ups_2=UpSample(64,True)
# 		self.ups_3=UpSample(32,True)
# 		self.ups_4=UpSample(3,True)
...
# 		###-------Dencoder------###
# 		out_dec3 = self.decoders[0](self.reduces1(torch.cat([(self.ups_1(out_enc_rgb4)), self.attention[0](out_enc_rgb3)], dim=1)))
# 		# print(out_dec3.shape)
# 		out_dec2 = self.decoders[1](self.reduces2(torch.cat([self.ups_2(out_dec3),self.attention[1](out_enc_rgb2)], dim=1)))
...
class mymodel(nn.Module):
	...
	def forward(self,RGB_input):
		...
		# print(fr.shape)  
		outi=self.ups1(fr) 
		# print(self.output(self.outputl(fr)).shape)
		# print(self.output(self.output1(outi)).shape)
		# print(outi.shape)
